...
interface ProjectData {
  ...
  meta: { lastUpdated: string; totalProjects: number };
}
...
function getSessionAccentHue(): number {
...
function resolveImageUrl(image: ProjectImage): string {
  if (image.type === "thum") {
...
function buildEntry(
  ...
  article.addEventListener("click", (e) => {
    ...
    if ((e.target as HTMLElement).closest(".entry-action")) return;
    if (isTouchDevice()) {
...
function buildImageHtml(project: Project): string {
...
function buildActionsHtml(project: Project): string {
  const parts: string[] = [];

  if (project.url) {
    parts.push(
      `<a href="${escAttr(project.url)}" class="entry-action entry-action--primary"
  ...
  if (project.repo) {
    parts.push(
      `<a href="${escAttr(project.repo)}" class="entry-action entry-action--secondary"
         target="_blank" rel="noopener noreferrer"
...
// ── Filter logic ─────────────────────────────────────────────
function setupFilters(allProjects: Project[]): void {
  ...
  tabsContainer.addEventListener("click", (e) => {
    ...
    // Update tab states
    tabsContainer.querySelectorAll<HTMLButtonElement>(".filter-tab").forEach((t) => {
    ...
    // Show/hide entries
    const entries = document.querySelectorAll<HTMLElement>(".project-entry");
    let visibleCount = 0;
...
async function init(): Promise<void> {
  ...
  try {
    ...
    data = await resp.json() as ProjectData;
  } catch (err) {
    console.error("Failed to load projects.json:", err);
    const grid = document.getElementById("project-grid");
    if (grid) grid.innerHTML = `<p style="color:var(--text-secondary);padding:2rem 0">Could not load projects.</p>`;
    return;
  }
