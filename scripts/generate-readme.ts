...
function readmeImageUrl(project: Project): string {
...
function buildFeaturedTable(projects: Project[]): string {
  ...
  for (let i = 0; i < featured.length; i += 3) {
    ...
    while (chunk.length < 3) chunk.push(null as unknown as Project);
...
const data: ProjectData = JSON.parse(readFileSync(dataPath, "utf-8"));
